...
public class AVLTree<T extends Comparable<T>> implements Tree<T> {
	...
	private void insert(T data, Node<T> node) {
		...
		if (node.getData().compareTo(data) > 0) {
			...
			if (node.getRigthChild() != null)
				insert(data, node.getRigthChild());
	...
	private void remove(T data, Node<T> node) {
		...
		if (data.compareTo(node.getData()) < 0) {
			...
			remove(data, node.getRigthChild());
		} else {
			...
			if (node.getLeftChild() == null && node.getRigthChild() == null) {
				...
				} else if (parent != null && parent.getRigthChild() == node) {
			...
			else if (node.getLeftChild() == null && node.getRigthChild() != null) {
				System.out.println("Removing a node with a single right child ...");
				Node<T> parent = node.getParentNode();

				// the node is a left child
				if (parent != null && parent.getLeftChild() == node) {
					parent.setLeftChild(node.getRigthChild());
					// the node is a right child
				} else if (parent != null && parent.getRigthChild() == node) {
					parent.setRightChild(node.getRigthChild());
				}

				// when we deal with the root node
				if (parent == null) {
					root = node.getRigthChild();
				}

				// have to update the right child's parent
				node.getRigthChild().setParentNode(parent);
				node = null;
	...
	private Node<T> getPredecessor(Node<T> node) {
		...
		if (node.getRigthChild() != null)
			return getPredecessor(node.getRigthChild());
	...
	private void settleViolationsHelper(Node<T> node) {
		...
		// OK, we know the tree is RIGHT HEAVY BUT it can be right-right heavy or
		// right-left heavy
		if (balance < -1) {
			// right left heavy situation: right rotation
			if (getBalance(node.getRigthChild()) > 0) {
				rightRotation(node.getRigthChild());
			}
	...
	private void rightRotation(Node<T> node) {
		...
		// node C
		Node<T> grandChild = tempLeftChild.getRigthChild();
		...
		if (tempLeftChild.getParentNode() != null && tempLeftChild.getParentNode().getRigthChild() == node) {
	...
	private void leftRotation(Node<T> node) {
		...
		// this is the new root node after rotation (node B)
		Node<T> tempRightChild = node.getRigthChild();
		// node C
		...
		if (tempRightChild.getParentNode() != null && tempRightChild.getParentNode().getRigthChild() == node) {
	...
	private void updateHeight(Node<T> node) {
		node.setHeight(Math.max(height(node.getLeftChild()), height(node.getRigthChild())) + 1);
	}
	...
	private int getBalance(Node<T> node) {
		...
		return height(node.getLeftChild()) - height(node.getRigthChild());
	}
}
