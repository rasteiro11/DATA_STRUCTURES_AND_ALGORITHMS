public class HeapConverter {
   ...
   // represent the heap with one dimensional array
   private int[] heap;
   ...
   public int[] transform() {
      ...
      for (int i = ((heap.length - 2) / 2); i >= 0; i--) {
         // we "heapify" all the internal nodes: we check weather the parent is smaller
         // than the children
   ...
   private void heapify(int index) {
      ...
      // check the left child weather it is smaller than the parent
      if (leftChildIndex < heap.length && heap[leftChildIndex] < heap[index])
         smallest = leftChildIndex;

      // check the right child weather it is smaller than the parent
