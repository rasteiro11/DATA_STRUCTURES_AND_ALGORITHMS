...
/**
 * At the insertion, it always is going to be from the left to the right.
 *
 * At every insertion, it checks if the item is present or not in the Double
 * Linked List. If it is present, it is going to be moved to the Head (extreme
 * left side), and all items remain are going to be shifted to the right.
 *
 * Otherwise, that the item inserted is not present in the Double Linked List,
 * the last item Tail (extreme right) is going to be removed, and the new item
 * is going to be inserted at the head (extreme left side.)
 *
 * Least Recently Used (LRU) Cache, works with a Doble Linked List.
 *
 * Head at the left side, and tail to the right side.
 */
...
public class LRUCache {
    ...
    private void add(Node node) {
        ...
        // we have to update the previous head: point to the new head (which is the
        // node)
